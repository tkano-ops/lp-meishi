...
3. `/create-lp` で**仮組み**のLPページを生成する（構成・コピー・配色は確定させるが、
   本人の実写真はまだ入れず空欄でよい／アニメーションは最小限）。
   これをクライアントに共有し、構成・コピー・デザインの大枠を確認してもらう
4. フィードバックを反映して仮組みを修正する（無料修正は2回まで。`docs/料金表.md` 参照）
5. 承認が取れたら`/create-lp`で**本仕上げ**を行う（本人の実写真に差し替え、
   Framer Motionでスクロール演出・ホバーなどの動きを作り込む）
6. `/review` で最終チェック
7. クライアントに最終確認してもらい、必要なら微修正
8. 納品（NFCカード・QR発行）
9. 保守・運用（月額プラン、`docs/料金表.md` 参照）
